  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Analysis Archive</h2>
          <p className="text-slate-500 mt-1">Access and review your previously processed genomic records.</p>
        </div>
      ...
      {history.length === 0 ? (
        <div className="bg-white rounded-3xl border border-dashed border-slate-300 p-20 text-center space-y-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900">Your archive is empty</h3>
            <p className="text-slate-500 max-w-xs mx-auto">Upload a genome assembly to begin populating your persistent history.</p>
          </div>
          <button 
            onClick={onBack}
        ...
        <div className="space-y-4">
          {history.map((item) => (
            <div 
              key={item.id}
              className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-emerald-300 hover:shadow-md transition-all group flex items-center justify-between"
            >
              <div className="flex items-center space-x-5">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 text-lg group-hover:text-emerald-700 transition-colors">{item.fileName}</h4>
                  <div className="flex items-center space-x-4 text-xs text-slate-400 font-medium">
                    ...
                    <span className="px-2 py-0.5 bg-slate-100 rounded-full text-slate-500">{item.result.selectedTools.length} Modules</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-4">
                <div className="hidden sm:flex flex-col text-right pr-4 border-r border-slate-100">
                  ...
                  <span className="text-xs font-bold text-slate-700">{item.result.stats.gcContent.toFixed(1)}% GC • {(item.result.stats.n50 / 1000).toFixed(1)}k N50</span>
                </div>
                ...
                <button 
                  ...
                  className="flex items-center px-4 py-2 bg-slate-900 text-white rounded-xl font-bold hover:bg-emerald-600 transition-all shadow-sm"
                >
